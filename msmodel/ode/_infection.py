...
def ODE(t, conc, drugdose, ke_diacid,
    ...
    k_in = 0.1
    if is_infected:
        d_k_ACE2_dt = k_in * AngII_conc - np.log(2) / 2 * k_ACE2
    else:
        d_k_ACE2_dt = 0
    ...
    d_IR_dt = 0.15 * (k_ACE2 - k_ACE2_0) + 0.001 * diacid_conc + 0.1 * glu - np.log(2) / 1 * IR * k_out

    # d_IR_dt = d_k_ACE2_dt + 0.02 * diacid_conc * glu - np.log(2) / 2 * IR
